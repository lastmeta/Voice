...
class OpenAIClient:
    ...
    recent_topics = []
    max_recent_topics = 10  # Increased from 5 to 10
...
2. Offer unique insights that challenge conventional thinking and spark discussion
3. Explain complex ideas clearly, balancing depth with accessibility 
4. Make bold, reasoned predictions about future tech developments
5. Write with confidence, reflection, and a cypherpunk point of view
6. Include 1-2 specific, relevant hashtags
7. Aim for 200-280 characters, optimized for engagement
...
Recent topics to avoid: {recent_topics_str}. Please choose a different topic that hasn't been covered recently. Aim for a balanced mix of your core themes over time."""
            ...
            response = client.chat.completions.create(
                ...
                n=1,
                temperature=0.8,  # Increased from 0.7 to 0.8 for more variety
            )
    ...
    def generate_tweet_with_hashtags():
        ...
        if tweet:
            ...
            if char_count(tweet) > 280:
                words = tweet.split()
                truncated = []
                current_length = 0
                hashtags = [word for word in words if word.startswith('#')]
                for word in words:
                    if word in hashtags or current_length + char_count(word) + 1 <= 277:
                        truncated.append(word)
                        current_length += char_count(word) + 1
                    else:
                        break
                tweet = ' '.join(truncated) + '...'
    ...
    @staticmethod
    def generate_hashtags(tweet_content, num_hashtags=2):
        try:
            response = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
            ...
            return ['#tech', '#innovation']  # Fallback hashtags
